...
public class RetailerCashoutSettlement extends BatchUtils {
	JavaUtils javaUtils = new JavaUtils();
	DBUtils dbUtils = new DBUtils();
	ApiUtils apiUtils = new ApiUtils();
	String workbook = "workbook", testDataSheetName = "RetailerCashOutSettlement", fileName, vanNumber, batchSheetName,
			batchConfigSection = "retailercashoutsettlement", cashOutWalletRefNo, cashOutWalletRemainingBal;
	String prevSettlementMode, currSettlementMode, prevRetailer, currRetailer;
	String responseCallbackConfigSection = "responsecallbackinvocation";
	...
	public void merchantWalletSettlementEODTest(HashMap<String, String> usrData) {
		...
		cashOutWalletRefNo = dbUtils.getAgentsWalletRefNo(usrData.get("Retailer")).get("cashOutWalletRefNo");
		String agentWalletRefNo = dbUtils.getAgentsWalletRefNo(usrData.get("Retailer")).get("agentWalletRefNo");
		cashOutWalletRemainingBal = apiUtils.getWalletBalance(cashOutWalletRefNo);
		apiUtils.depositAmountToWallet(cashOutWalletRefNo, usrData.get("Amount"));
		// batch start merchant settlement
		dbUtils.updateBatchConfigurationToSuccess(batchFileConfig.get("batch.code"));
		String exeSummary  = "walletToSettle="+cashOutWalletRefNo+", drPartyType=CASHOUT, stlMode=TO_BANK, crPartyType=RETAILER, walletToCredit="+agentWalletRefNo;
	    batchFileConfig.put("batch.execution.summary", exeSummary);
		String line = executeBatch(batchFileConfig);
		System.out.println(line);
		dbUtils.stopBatchConfiguration(batchFileConfig.get("batch.code"));
		// start the batch

		// batch response callback
		responseBatchConfig = javaUtils.readSectionFromIni(responseCallbackConfigSection);
